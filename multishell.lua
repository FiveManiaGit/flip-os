...
if parentTerm.isColor() then
    ...
    menuOtherTextColor, menuOtherBgColor = colors.black, colors.lightBlue
else
...
local function redrawMenu()
    if bShowMenu then
        ...
        for n = nScrollPos, #tProcesses do
            ...
            table.insert(procT, string.upper(string.sub(tProcesses[n].sTitle, 1, 1)))

            for i = 2, string.len(tProcesses[n].sTitle) do
              table.insert(procT, string.sub(tProcesses[n].sTitle, i, i))
            end

            parentTerm.write(" " .. table.concat(procT) .. " ")
            nCharCount = nCharCount + #tProcesses[n].sTitle + 4
        end
        ...
        if nScrollPos ~= 1 then
            ...
            parentTerm.setBackgroundColor(colors.white)
...
function multishell.setMenuVisible(bVis)
  if bShowMenu ~= bVis then
      bShowMenu = bVis
      resizeWindows()
      redrawMenu()
...
while #tProcesses > 0 do
    ...
    elseif sEvent == "mouse_click" then
        ...
        if bShowMenu and y == 1 then
            -- Switch process
            if x == 2 and nScrollPos ~= 1 then
                nScrollPos = nScrollPos - 1
                redrawMenu()
            elseif x == 1 and nScrollPos == 1 then
            ...
            elseif bScrollRight and x == term.getSize() then
                ...
                for n = nScrollPos, #tProcesses do
                    local tabEnd = tabStart + #tProcesses[n].sTitle + 2
